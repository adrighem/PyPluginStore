...
Public registry packages do not expose a Release-to-Git channel switch. A verified migration backup appears as **Restore Git**. A retained Release backup names its target as **Restore vX**; the compact fallback is **Rollback** when no exact version can be shown. The confirmation dialog always describes the full restore target. Restoring a Git backup records a safety hold so the same Release is not immediately reapplied. Add a local registry override for ongoing branch-based Git updates. An override does not turn an existing Release folder into a Git checkout: restore a verified Git backup first, or remove and reinstall the plugin after adding the override.
